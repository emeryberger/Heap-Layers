...
template <class Header, size_t Size> class bins;
...
public:
  bins() {
    ...
    static_assert(getClassSize(0) < getClassSize(1),
                  "Need distinct size classes.");
    static_assert(getSizeClass(getClassSize(0)) == 0,
                  "Size class computation error.");
    static_assert(getSizeClass(0) >= getSizeClass(alignof(max_align_t)),
                  "Min size must be at least alignof(max_align_t).");
#ifndef NDEBUG
    int bins = 0;
...
private:
  // constexpr integer log base two calculations, ONLY for use during
  // compilation.
...
public:
  ...
  static inline constexpr int getSizeClass(size_t sz) {
    ...
    auto sizeClass = (int)HL::ilog2(sz) -
                     LogMaxAlignT; // (int) HL::ilog2(alignof(max_align_t));
    return sizeClass;
  }
