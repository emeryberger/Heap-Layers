 ...
 * @note   Copyright (C) 2011-2015 by Emery Berger, University of Massachusetts
 * Amherst.
 */
...
void *WINWRAPPER_PREFIX(_expand)(void *ptr) { return nullptr; }

void *WINWRAPPER_PREFIX(_expand_dbg)(void *userData, size_t newSize,
...
static void *WINWRAPPER_PREFIX(_recalloc)(void *memblock, size_t num,
                                          size_t size) {
  ...
  if (ptr != nullptr) {
    ...
    if (actualSize > requestedSize) {
      ...
      memset(static_cast<char *>(ptr) + requestedSize, 0,
...
void WINWRAPPER_PREFIX(_cexit)() { executeRegisteredFunctions(); }
...
void *WINWRAPPER_PREFIX(_calloc_dbg)(size_t num, size_t size, int blockType,
                                     const char *filename, int linenumber) {
  return WINWRAPPER_PREFIX(calloc)(num, size);
}

void *WINWRAPPER_PREFIX(_malloc_dbg)(size_t size, int blockType,
                                     const char *filename, int linenumber) {
  return xxmalloc(size);
}

void *WINWRAPPER_PREFIX(_realloc_dbg)(void *userData, size_t newSize,
                                      int blockType, const char *filename,
                                      int linenumber) {
  return WINWRAPPER_PREFIX(realloc)(userData, newSize);
}

void WINWRAPPER_PREFIX(_free_dbg)(void *userData, int blockType) {
  xxfree(userData);
}

size_t WINWRAPPER_PREFIX(_msize_dbg)(void *userData, int blockType) {
  return xxmalloc_usable_size(userData);
}

LPVOID WINWRAPPER_PREFIX(HeapAlloc)(HANDLE hHeap, DWORD dwFlags,
                                    SIZE_T dwBytes) {
  if (hHeap == nullptr) {
    return nullptr;
  }
...
SIZE_T WINAPI WINWRAPPER_PREFIX(HeapCompact)(HANDLE hHeap, DWORD dwFlags) {
...
BOOL WINWRAPPER_PREFIX(HeapFree)(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem) {
  xxfree(lpMem);
  return true;
}

LPVOID WINAPI WINWRAPPER_PREFIX(HeapReAlloc)(HANDLE hHeap, DWORD dwFlags,
                                             LPVOID lpMem, SIZE_T dwBytes) {
  // Immediately fail if we are asked to realloc in place (since we can't
  // guarantee it).
...
BOOL WINAPI WINWRAPPER_PREFIX(HeapValidate)(HANDLE hHeap, DWORD dwFlags,
                                            LPCVOID lpMem) {
  // A stub that says the heap is fine.
  return TRUE;
}

SIZE_T WINAPI WINWRAPPER_PREFIX(HeapSize)(HANDLE hHeap, DWORD dwFlags,
                                          LPCVOID lpMem) {
...
PVOID WINWRAPPER_PREFIX(RtlAllocateHeap)(PVOID HeapHandle, ULONG Flags,
                                         SIZE_T Size) {
...
ULONG
WINWRAPPER_PREFIX(RtlSizeHeap)
(PVOID HeapHandle, ULONG Flags, PVOID MemoryPointer) {
  return xxmalloc_usable_size(MemoryPointer);
}

PVOID WINWRAPPER_PREFIX(RtlCreateHeap)(
    ULONG Flags, PVOID HeapBase, SIZE_T ReserveSize, SIZE_T CommitSize,
    PVOID Lock, PVOID /* PRTL_HEAP_PARAMETERS */ Parameters) {
  // FIX ME: right now, it's just a bogus number.
  return (PVOID)0x1;
}

BOOLEAN WINWRAPPER_PREFIX(RtlFreeHeap)(PVOID HeapHandle, ULONG Flags,
                                       PVOID HeapBase) {
  xxfree(HeapBase);
  return TRUE;
}

PVOID WINWRAPPER_PREFIX(RtlDestroyHeap)(PVOID HeapHandle) { return nullptr; }
}

/* ------------------------------------------------------------------------ */

#define INTERPOSE(x)                                                           \
  { #x, (FARPROC) winwrapper_##x, false, 0 }
#define INTERPOSE2(x, y)                                                       \
  { x, (FARPROC)y, false, 0 }
...
static Patch rls_patches[] = {
    ...
    //    {"RtlCreateHeap",  (FARPROC) WINWRAPPER_PREFIX(RtlCreateHeap),
    //    false, 0},
    //    {"RtlDestroyHeap", (FARPROC) WINWRAPPER_PREFIX(RtlDestroyHeap),
    //    false, 0},
...
static bool PatchMe() {
  ...
  // We walk through all modules loaded by the process and patch the relevant
  // entry points.
  ...
  if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
    for (auto i = 0; i < (cbNeeded / sizeof(HMODULE)); i++) {
      ...
      if (GetModuleFileName(hMods[i], pszBuffer,
        ...
        if (!(_tcsstr(pszBuffer, _T("CRT")) || _tcsstr(pszBuffer, _T("crt")))) {
          continue;
        }
        ...
        if (RlsCRTLibrary) {
          for (auto j = 0; j < sizeof(rls_patches) / sizeof(*rls_patches);
               j++) {
            if (rls_patches[j].original =
                    GetProcAddress(RlsCRTLibrary, rls_patches[j].import)) {
              // Got one: patch it.
