...
template <class T, class Super> class STLAllocator : public Super {
public:
  ...
  STLAllocator(const STLAllocator &s) throw() : Super(s) {}
  ...
  inline void *allocate(size_type n, const void * = 0) {
    if (n) {
  ...
  inline void deallocate(void *p, size_type) { Super::free(p); }

  inline void deallocate(pointer p, size_type) { Super::free(p); }
...
inline bool operator!=(const STLAllocator<T, S> &a,
