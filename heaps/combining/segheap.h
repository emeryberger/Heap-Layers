...
template <int NumBins, int (*getSizeClass)(const size_t),
          size_t (*getClassMaxSize)(const int), class LittleHeap, class BigHeap>
class SegHeap : public LittleHeap {
public:
  enum { Alignment = gcd<LittleHeap::Alignment, BigHeap::Alignment>::VALUE };

  inline SegHeap()
      : _memoryHeld(0), _maxObjectSize(getClassMaxSize(NumBins - 1)) {
    for (int i = 0; i < NUM_ULONGS; i++) {
      binmap[i] = 0;
    }
  }

  inline size_t getMemoryHeld() const { return _memoryHeld; }

  size_t getSize(void *ptr) { return LittleHeap::getSize(ptr); }
