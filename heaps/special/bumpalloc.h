...
public:
  ...
  BumpAlloc() : _bump(NULL), _remaining(0) {
    ...
    static_assert(
        (int)gcd<SuperHeap::Alignment, Alignment>::VALUE == Alignment,
...
// We are going to rename this layer to BumpHeap and deprecate BumpAlloc
// eventually.
template <size_t Size, class X> using BumpHeap = BumpAlloc<Size, X>;
}
