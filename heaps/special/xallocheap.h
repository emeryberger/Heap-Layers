...
template <int ArenaSize, class SuperHeap> class XallocHeap : public SuperHeap {
...
public:
  ...
  inline ~XallocHeap() { SuperHeap::free(start_of_array); }
...
private:
  ...
  static inline int in_use(char *x) { return (size_lval(x) & (1)); }

  static inline void set_use(char *x) { (size_lval(x) |= (1)); }

  static inline void clear_use(char *x) { (size_lval(x) &= (~1)); }
