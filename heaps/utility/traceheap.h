...
class FileObject {
public:
  FileObject() : _isOpen(false), _file(0) {}

  FileObject(const FileObject &f) : _isOpen(f._isOpen), _file(f._file) {}
...
template <class Super, int Number> class TraceHeap : public Super {
public:
  TraceHeap() {
    if (!theFile().is_open()) {
      char fname[255];
