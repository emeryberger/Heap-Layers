...
class WinMallocHeap {
public:
  static inline void *malloc(size_t sz) { return malloc(sz); }

  static inline void free(void *p) { free(p); }

  static inline size_t getSize(const void *p) { return _msize((void *)p); }
};
...
extern "C" void regionFreeAll(void **reg) { ((PseudoRegion *)*reg)->freeAll(); }
...
extern "C" void *obstack_copy(struct obstack *obstack, void *address,
                              size_t size) {
  ObstackType *ob = (ObstackType *)obstack->theObstack;
...
extern "C" void *obstack_copy0(struct obstack *obstack, void *address,
                               size_t size) {
  ObstackType *ob = (ObstackType *)obstack->theObstack;
